...
impl Demodulation {
    ...
    pub fn run(&self, arr: Vec<Complex<f32>>) -> Vec<u8>
    {
        ...
        // run fft
        let mut planner = FftPlanner::new();
        let fft = planner.plan_fft_forward(self.samples_per_symbol);
        fft.process_with_scratch(arr_clone.as_mut_slice(), scratch.as_mut_slice());
        ...
        if self.samples_per_symbol >= 2_i32.pow(MFSK_BITS_ENCODED as u32) as usize {
            for x in 0..2_i32.pow(MFSK_BITS_ENCODED as u32) {
                let index = ((x * transmission_window) as f32 / fft_idk).round() as usize;
        ...
        for x in chunks {
            ...
            bin += mfsk_fft_index_map[index] as u8;
