...
impl graphy
{
    pub fn graph(file_name: &str, arr: Vec<Complex<f32>>) -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        chart
            .draw_series(LineSeries::new(
                (0..arr.len()).map(|x| x as f32).map(|x| (x, 20.0 * ((arr.get(x as usize).unwrap().re.powf(2.0) + arr.get(x as usize).unwrap().im.powf(2.0)).sqrt()) )),
                &RED,
            ))?
            .label("y = x^2")
    ...
    pub fn graph_vec(file_name: &str, arr: Vec<f32>) -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        chart
            .draw_series(LineSeries::new(
                (0..arr.len()).map(|x| x as f32).map(|x| (x, *arr.get(x as usize).unwrap() )),
                &RED,
            ))?
            .label("y = x^2")
            .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], &RED));
