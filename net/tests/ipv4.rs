...
pub fn ipv4_address() {
    ...
    assert_eq!(
        test2.to_string(),
        "192.168.1.4".to_string(),
        "Failed to encode and decode"
    );
    assert_eq!(
        test3.to_string(),
        "192.168.1.4".to_string(),
        "Failed to create from verbatim"
    );
}
...
pub fn encode_decode() {
    ...
    assert_eq!(
        decode.differentiated_services_code_point,
        x.differentiated_services_code_point
    );
    assert_eq!(
        decode.explicit_congestion_notification,
        x.explicit_congestion_notification
    );
